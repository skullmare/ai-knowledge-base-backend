...
Сама модель мультимодальная, но **эндпоинт эмбеддингов** RouterAI работает по
обычной OpenAI-схеме: по спецификации (`/api/openapi.json`) поле `input` — это
«строка или массив строк», а на файловый ввод приходит
`invalid_union: expected string, received array`. Поэтому для индексации текст
из документов извлекается на своей стороне, в процессе, без внешнего сервиса
разбора (пакет `officeparser`).

Это ограничение именно эмбеддингов. Эндпоинт `/v1/chat/completions` файлы
принимает — проверено на PDF, DOCX и XLSX, модель отвечает по их содержимому:

```json
{ "type": "file", "file": { "filename": "doc.pdf", "file_data": "data:application/pdf;base64,…" } }
```

Для векторного поиска это не подходит (вектор строится только из текста), но
годится как запасной путь для форматов, которые `officeparser` не читает.
...
| изображения, аудио, видео | не индексируются — вектор строится только из текста |
