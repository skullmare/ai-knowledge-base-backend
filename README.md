...
| `.pdf`, `.docx`, `.xlsx`, `.pptx`, `.odt`, `.rtf`, `.epub`… | текст извлекается через `officeparser` |
...
отдельной точкой в Qdrant — в контекст агента попадает сам текст фрагмента.
Отсканированные PDF без текстового слоя векторизовать нельзя.
